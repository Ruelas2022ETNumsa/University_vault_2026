...
Para ejercicios AHPL — el orden es siempre: tabla de declaraciones → bloque de código → tabla de pasos.

1) Tabla de declaraciones — SIEMPRE primero, antes del bloque de código.
Listar todos los identificadores usados en el módulo: registros, buses, señales.
Una fila por identificador. Columnas exactas:

| Identificador | Sección | Tamaño | Rol |
|-|-|-|-|
| `DR` | MEMORY | `[18]` | descripción del rol en este módulo |
| `first` | MEMORY | escalar | descripción |
| `IOBUS` | COMBUS | `[18]` | descripción |
| `ready` | OUTPUTS | escalar | descripción |
| `datavalid` | INPUTS | escalar | descripción |

Reglas de la tabla de declaraciones:
- Sección: MEMORY · OUTPUTS · INPUTS · COMBUS — tal como aparece en las declaraciones del módulo.
- Tamaño: `[N]` para vectores (con corchetes en backticks) · `escalar` para flip-flops y señales de 1 bit.
  Nunca omitir el tamaño de un vector. Nunca escribir `[1]` — si es 1 bit es escalar.
- Rol: descripción funcional breve — qué hace ese registro o señal en este módulo específico.
- Incluir todos los identificadores que aparecen en MEMORY, OUTPUTS, INPUTS y COMBUS.
- No incluir identificadores que no estén declarados (no inventar registros intermedios).

2) Bloque de código — después de la tabla de declaraciones.
Módulo completo con declaraciones y pasos numerados, en bloque de código estándar.

3) Tabla de pasos — después del bloque de código.
