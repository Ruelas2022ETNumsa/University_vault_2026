...
| HP Prime Virtual Calculator | 2025-09-15 (x64) | Emulador completo — nombrado CASE |

### Dispositivos en el Connectivity Kit

| Nombre | Tipo | Conexión |
|---|---|---|
| TARS | HP Prime G2 física | USB |
| CASE | HP Prime Virtual Calculator | Red IP local |

> CASE se renombró desde `Shift + Home → Página 2/4 → Nombre de calculadora`.
...
```
Claude escribe el programa en HP PPL
        ↓
Lo guarda en E:\University_vault_2026\_app\Tars\work\
        ↓
Connectivity Kit detecta el archivo
        ↓
Se transfiere a TARS con un clic (o drag & drop)
```

### Conexión CASE ↔ Connectivity Kit (red IP local)

La opción "Habilitar red" en Editar → Preferencias de la calculadora virtual
permite que el Connectivity Kit detecte CASE como si fuera una segunda
calculadora conectada, sin necesidad de USB.

- Activar en **ambos programas**: Virtual Calculator y Connectivity Kit
- Tener ambos abiertos al mismo tiempo → el Kit detecta CASE en segundos
- Si el firewall de Windows pregunta → permitir acceso
- TARS (USB) y CASE (red IP) pueden estar conectadas simultáneamente sin conflicto
