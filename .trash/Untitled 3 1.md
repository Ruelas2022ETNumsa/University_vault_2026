...
```cornell
::cue
valor absoluto · distancia · módulo · magnitud

¿Cómo se define el valor absoluto de un número real de forma analítica?

¿Qué interpretación geométrica tiene el valor absoluto en la recta real?

¿Cuáles son las propiedades fundamentales que rigen las operaciones con barras de valor absoluto?

$|x| = \sqrt{x^2}$

ver también: [[intervalos]]

no confundir $|x| < a$ con la unión de intervalos.
...
$$\begin{array}{rcll}
  -6 & = & -6 \\
  4 - 10 & = & 9 - 15 \\
  2^2 - 2 \cdot 2 \cdot \frac{5}{2} + \left(\frac{5}{2}\right)^2 & = & 3^2 - 2 \cdot 3 \cdot \frac{5}{2} + \left(\frac{5}{2}\right)^2 & (\text{completando cuadrados}) \\
  \left( 2 - \frac{5}{2} \right)^2 & = & \left( 3 - \frac{5}{2} \right)^2 \\
  \sqrt{\left( 2 - \frac{5}{2} \right)^2} & = & \sqrt{\left( 3 - \frac{5}{2} \right)^2} \\
  \left| 2 - \frac{5}{2} \right| & = & \left| 3 - \frac{5}{2} \right| & (\text{aplicando prop. 7}) \\
...
> **Resumen:** El valor absoluto define la magnitud no negativa de un número real, representable como su distancia al origen y sujeta a propiedades algebraicas de orden y desigualdad.
...
```
mnemónico: **S**implemente **P**ara **M**i **D**iversión **I**ntenté **C**omprar **R**opa *
```