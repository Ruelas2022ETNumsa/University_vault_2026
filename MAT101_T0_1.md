...
El punto clave aquí es el **orden de los pasos 2 y 3**. Mucha gente los mezcla, pero primero tienes que *agregar* $(-a)$ (Axioma 5 garantiza que existe), y *luego* reagrupar con asociatividad (Axioma 3). No puedes reagrupar algo que todavía no está en la expresión.
...
El paso más elegante aquí es el 4, que parece trivial pero es necesario. Para poder aplicar la cancelación aditiva del Ejercicio 1 necesitas que ambos lados tengan la misma forma $x + \square = x + \square$. El paso 4 reescribe el lado derecho como $a \cdot 0 + 0$ para que la cancelación aplique limpiamente.
...
- el factor $(x-1)$ se cancela, pero **no desaparece del dominio**. Mucha gente simplifica y escribe $f(x) = \frac{x+2}{x}$ sin aclarar que $x \neq 1$ sigue siendo una restricción. Eso es un error frecuente en exámenes.
