...
|                      | $5/0$                   | $0/0$                           |
| -------------------- | ----------------------- | ------------------------------- |
| ¿Tiene solución?     | Ninguna                 | Cualquiera                      |
| Nombre               | **Indefinido**          | **Indeterminado**               |
| ¿Por qué falla?      | Contradicción ($0 = 1$) | Ambigüedad (infinitas opciones) |
| ¿Aparece en cálculo? | Asíntota vertical       | Límites tipo $0/0$              |
>- **Indefinido/imposible:** $5/0$ — no existe ningún número real que funcione. El sistema colapsa con una contradicción. No hay nada que analizar, simplemente no existe.
>- **Indeterminado:** $0/0$ — aquí el problema es diferente: *cualquier* número podría funcionar, entonces hay infinitas "soluciones" y no podemos determinar cuál es. Por eso se llama indeterminación (esto lo vas a ver mucho en límites).
...
1. ¿Por qué es necesario usar la propiedad conmutativa en estas demostraciones?
	R.- Es el **puente lógico** que une ambas ecuaciones en la demostración. Sin esta la demostracion se rompe an algo como  $0' + 0 \neq 0 + 0'$ 
...
2. Si un conjunto no cumpliera con el axioma de conmutatividad, ¿podría tener más de un elemento neutro (uno por la izquierda y otro por la derecha)?
	R.- Si, **sin conmutatividad prdian exisitir neutros distintos por izquierda y derecha.**
3. ¿Cómo cambiaría la demostración si intentamos probar la unicidad de los inversos (opuestos)?
	R.- Supón que $a$ tiene dos opuestos, llamémoslos $(-a)$ y $(-a)'$. Entonces:
	$$(-a)' = (-a)' + 0$$
	$$= (-a)' + (a + (-a))$$
	$$= ((-a)' + a) + (-a)$$
	$$= 0 + (-a)$$
	...
	Entonces $(-a)' = (-a)$, es decir, el opuesto es único.
	
4. Según los axiomas de cuerpo, ¿cómo se define formalmente la operación de división?
	R.-Dividir b entre a **no es una operación primitiva** — es una abreviación de multiplicar b por el recíproco de a, que existe y es único por el Axioma 6
5. Si la división por cero no está definida, ¿qué sucede con la gráfica de una función racional cuando su denominador se acerca a cero?
	R.-  Si, $f(x)=1/x$ ocurren 2 cosas, visto desde el eje de coordenadas.
	- Desde derecha si ($x \to 0^+$): $f(x) \to +\infty$ 
	- Desde la izquierda ($x \to 0^-$): $f(x) \to -\infty$
6. ¿Por qué el "cuerpo trivial" (donde $1 = 0$) no es útil para el cálculo en ingeniería?
	Si $1 = 0$, entonces para cualquier $a$:
	$$a = a \cdot 1 = a \cdot 0 = 0$$
	Todos los números son el mismo número. No puedes representar "5 kg" distinto de "0 kg" ni de "1000 kg". Todo es lo mismo. Para ingeniería eso es inútil — necesitas que los números distintos *sean* distintos, y eso lo garantiza exactamente la condición $1 \neq 0$ del Axioma 4.

>En resumen, el hilo conductor de ambas preguntas 1,2 es el mismo: la conmutatividad no es decorativa, **es lo que hace que los neutros sean únicos y que las demostraciones cierren**.
...
Hipótesis: $a + b = a + c$. Queremos demostrar que $b = c$.

| Paso | Expresión                         | Justificación                                  |
| ---- | --------------------------------- | ---------------------------------------------- |
| 1    | $a + b = a + c$                   | Hipótesis                                      |
| 2    | $(-a) + (a + b) = (-a) + (a + c)$ | Axioma 5: existe $(-a)$, se suma a ambos lados |
| 3    | $((-a) + a) + b = ((-a) + a) + c$ | Axioma 3: asociatividad de la suma             |
| 4    | $0 + b = 0 + c$                   | Axioma 5: $(-a) + a = 0$                       |
| 5    | $b = c$                           | Axioma 4: el 0 es neutro aditivo, $0 + x = x$  |
...
Queremos demostrar que $a \cdot 0 = 0$.

| Paso | Expresión                               | Justificación                                                           |
| ---- | --------------------------------------- | ----------------------------------------------------------------------- |
| 1    | $0 + 0 = 0$                             | Axioma 4: el 0 es neutro aditivo                                        |
| 2    | $a \cdot (0 + 0) = a \cdot 0$           | Se multiplica ambos lados por $a$                                       |
| 3    | $a \cdot 0 + a \cdot 0 = a \cdot 0$     | Axioma 7: distributividad                                               |
| 4    | $a \cdot 0 + a \cdot 0 = a \cdot 0 + 0$ | Axioma 4: $a \cdot 0 = a \cdot 0 + 0$ (neutro aditivo)                  |
| 5    | $a \cdot 0 = 0$                         | Ejercicio 1: cancelación aditiva, se cancela $a \cdot 0$ de ambos lados |
...
- el factor $(x-1)$ se cancela, pero **no desaparece del dominio**. 

La función simplificada es:

$$f(x) = \frac{x+2}{x}, \quad x \neq 0 \text{ y } x \neq 1$$

| Valor | ¿Por qué se excluye? |
|---|---|
| $x = 0$ | Hace el denominador $= 0$ directamente |
| $x = 1$ | Hacía $0/0$ antes de simplificar — agujero en la gráfica |

$$\therefore \text{Dominio} = \mathbb{R} \setminus \{0, 1\}$$

La diferencia entre $x=0$ y $x=1$ es relevante: en $x=0$ hay una asíntota vertical, en $x=1$ hay un agujero removible. Los dos excluyen el punto, pero por razones distintas que verás cuando llegues a límites.
